...
void TracerBase::UpdateTracerState(const TracerMessage &status_msg, TracerState &state)
{
    ...
    case TracerMotorDriverStatusMsg:
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            state.actuator_states[status_msg.body.motor_heigh_speed_msg.motor_id].motor_rpm = static_cast<int16_t>(static_cast<uint16_t>(msg.data.status.rpm.low_byte) | static_cast<uint16_t>(msg.data.status.rpm.high_byte) << 8);
    ...
    case TracerOdometerMsg:
    {
        ...
        state.right_odometry=static_cast<int32_t>((static_cast<uint32_t>(msg.data.status.rightodometer.lowest))|(static_cast<uint32_t>(msg.data.status.rightodometer.sec_lowest)<<8)|(static_cast<uint32_t>(msg.data.status.rightodometer.sec_highest)<<16)|(static_cast<uint32_t>(msg.data.status.rightodometer.highest)<<24));
        state.left_odometry=static_cast<int32_t>((static_cast<uint32_t>(msg.data.status.leftodometer.lowest))|(static_cast<uint32_t>(msg.data.status.leftodometer.sec_lowest)<<8)|(static_cast<uint32_t>(msg.data.status.leftodometer.sec_highest)<<16)|(static_cast<uint32_t>(msg.data.status.leftodometer.highest)<<24));
